---
title: YearPicker
description: Memilih tahun dari daftar yang dapat digulir.
group: Form
playground: /playground/year-picker
---
...
```tsx
<YearPicker label="Year Picker" onChange={(nilai) => simpan(nilai)} />
```
...
```tsx
<YearPicker mode="single" />    // satu pilihan, bawaan
<YearPicker mode="multiple" />  // banyak pilihan
<YearPicker mode="range" />     // rentang antara dua nilai
```
