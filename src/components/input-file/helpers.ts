...
const getIconName = ({
  ...
  if (
    (type !== undefined && type?.includes('spreadsheet')) ||
    ext === 'xls' ||
    ext === 'xlsx'
  ) {
    return 'xls';
  ...
  if (
    (type !== undefined && type?.includes('word')) ||
    ext === 'doc' ||
    ext === 'docx'
  ) {
    return 'doc';
  ...
  if (
    (type !== undefined && type?.startsWith('video/')) ||
    ext === 'mkv' ||
    ext === 'mp4'
  ) {
    return 'mp4';
  ...
  if ((type !== undefined && type?.startsWith('audio/')) || ext === 'mp3') {
    return 'mp3';
  }

  if ((type !== undefined && type?.includes('pdf')) || ext === 'pdf') {
    return 'pdf';
  }
